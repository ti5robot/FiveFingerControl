...
public:
    HandControlNode() : Node("hand_control_node")
    {
        try
        {
            controller_ = std::make_unique<RS485Controller>("/dev/ttyUSB0");
            ...
            for(int i=0; i<4; i++) {
                finger_ranges_[i].min = 255;
                finger_ranges_[i].max = 0;
            }
...
private:
    ...
    FingerRange finger_ranges_[4]; // 只记录前4个手指的范围
    std::chrono::steady_clock::time_point calibration_start_;
    bool calibration_complete_ = false;

    uint8_t map_to_range(uint8_t value, uint8_t min, uint8_t max) {
        if(value <= min) return 0;
        if(value >= max) return 90;
        return static_cast<uint8_t>((value - min) * 90.0f / (max - min));
    }
    ...
    void command_callback(const std_msgs::msg::String::SharedPtr msg)
    {
        ...
        // if (right_hand[1] > 40 && right_hand[2] > 40 && right_hand[3] > 40)
        // {
        //     right_hand[0] = 80;
        // }
        // else if (right_hand[1] < 20 && right_hand[2] < 20 && right_hand[3] < 20)
        // {
        //     right_hand[0] = 20;
        // }
        ...
        //     if(right_hand[i]>15)
        //     {
        //          right_hand[i] = right_hand[i] * 1.1;
        //     }
           
           
        // }
        // right_hand[5] = right_hand[5] * 2.5;

        // 校准阶段（前10秒）
        auto now = std::chrono::steady_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::seconds>(now - calibration_start_);
        
        if(!calibration_complete_ && elapsed.count() < 11) {
            // 更新前4个手指的最小最大值
            for(int i=0; i<4; i++) {
                if(right_hand[i] < finger_ranges_[i].min) finger_ranges_[i].min = right_hand[i];
                if(right_hand[i] > finger_ranges_[i].max) finger_ranges_[i].max = right_hand[i];
            }
            
            if(elapsed.count() >= 10) {
                calibration_complete_ = true;
                RCLCPP_INFO(this->get_logger(), "校准完成，手指范围：");
                for(int i=0; i<4; i++) {
                    RCLCPP_INFO(this->get_logger(), "手指%d: [%d, %d]", i, 
                               finger_ranges_[i].min, finger_ranges_[i].max);
                }
            }
            return; // 校准阶段不发送控制命令
        }
        ...
        if(calibration_complete_) {
            for(int i=0; i<4; i++) {
                right_hand[i] = map_to_range(right_hand[i], 
                                           finger_ranges_[i].min, 
                                           finger_ranges_[i].max);
            }
            for(int i=0;i<4;i++)
            {
                int a=finger_ranges_[i].max;
                int b=finger_ranges_[i].min;
                cout << i+1 <<"最大值" << a << "最小值" << b << endl;
            }
        }
        right_hand[5] = right_hand[5] * 2.5;
